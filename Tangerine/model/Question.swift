...
public protocol isQuestion{
    ...
    /// is admin review requires ? default false
    var adminReviewRequired: Bool { get set }
...
public class Question : Codable, isQuestion{
    ...
    // Ref: https://docs.google.com/document/d/1Dt8ePoZwc634rPZBJG3ZLiORLRQvIiK_6jBHZoPpbGs/edit
    var usersNotConsumedBy = [String]()
    ...
    public var adminReviewRequired: Bool = false
    ...
    init(firebaseDict: [String:Any]) {
        ...
        // for seeders
        if let usersNotConsumedByArray = firebaseDict[Constants.QUES_USERS_NOT_CONSUMED_BY_KEY] as? [String]{
            var unrList = [String]()
            unrList.append(contentsOf: usersNotConsumedByArray)

            self.usersNotConsumedBy = unrList
        }else{
            print("error unwrapping downloaded usersNotConsumedBy list for \(self.question_name). \nStored an empty array to it instead: [String]()")
            self.usersNotConsumedBy = [String]()
        }
